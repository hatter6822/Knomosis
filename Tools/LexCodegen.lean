...
The `--gen-property-tests` flag emits a placeholder
`LegalKernel/Test/Properties/AutoGen.lean` summarising the
`(law, property)` pairs the codegen-input directory's `satisfies`
claims expose.

In M3 v1, the actual `AutoGen.lean` test file is hand-curated
(see the file's docstring) — the auto-generator's output is a
*manifest-of-coverage*: it lists which `(law, property)` pairs
SHOULD be exercised by the auto-generated test suite, so a
reviewer can compare against the hand-curated form and notice
gaps.

M4 may flip the gate to consume the generator's output directly
as the test file. -/

/-- Emit a coverage-manifest summary for the property-test
    auto-generator (LX.38).  One section per law, listing the
    `satisfies` claims that should map to per-law property
    tests.  Byte-stable across runs on equal inputs. -/
def emitPropertyTestManifest (decls : List LawDecl) : String := Id.run do
  let mut buf : String :=
    "# Auto-generated property-test coverage manifest\n\n" ++
    "# Generated by `lake exe lex_codegen --gen-property-tests`.\n" ++
    "# This is a coverage-of-claims summary; see\n" ++
    "# `LegalKernel/Test/Properties/AutoGen.lean` for the\n" ++
    "# hand-curated test suite (M3 v1).\n\n"
  let sortedDecls := decls.toArray.qsort
    (fun a b =>
      if a.actionIndex < b.actionIndex then true
      else if a.actionIndex > b.actionIndex then false
  ...
  for decl in sortedDecls do
    if !decl.satisfies.isEmpty then
      buf := buf ++ s!"## {decl.identifier}\n"
      for claim in decl.satisfies do
        buf := buf ++ s!"  - {claim.name}\n"
      buf := buf ++ "\n"
  pure buf
...
def main (args : List String) : IO UInt32 := do
  ...
  -- LX.38: --gen-property-tests mode.  Emits a coverage-manifest
  -- summary alongside the codegen-input directory.  In M3 v1,
  -- the AutoGen.lean test file is hand-curated; this mode emits
  -- the coverage summary for cross-check.
  if opts.genPropertyTests then
    IO.println "lex_codegen --gen-property-tests: M3 LX.38 coverage manifest"
    match (← loadCodegenInputs opts.inputDir) with
    | .error msg =>
      IO.eprintln s!"lex_codegen --gen-property-tests: {msg}"
      return 2
    | .ok decls =>
      let manifest := emitPropertyTestManifest decls
      let manifestPath : System.FilePath :=
        opts.inputDir / "property_test_coverage.txt"
      if opts.checkOnly then
        if (← manifestPath.pathExists) then
          let existing ← IO.FS.readFile manifestPath
          if existing == manifest then
            IO.println s!"lex_codegen --gen-property-tests --check: {decls.length} law(s); coverage manifest is byte-stable; OK"
            return 0
          else
            IO.eprintln s!"lex_codegen --gen-property-tests --check: coverage manifest divergence at {manifestPath.toString}"
            IO.eprintln "  re-run `lake exe lex_codegen --gen-property-tests` to regenerate"
            return 1
        else
          IO.eprintln s!"lex_codegen --gen-property-tests --check: coverage manifest does not exist at {manifestPath.toString}"
          return 1
      else
        match (← withFileLock manifestPath
                  (atomicWriteIfChanged manifestPath manifest)) with
        | none =>
          IO.eprintln s!"lex_codegen --gen-property-tests: another invocation holds the advisory lock for {manifestPath.toString}"
          return 1
        | some _ =>
          IO.println s!"lex_codegen --gen-property-tests: emitted coverage for {decls.length} law(s) to {manifestPath.toString}"
          return 0
