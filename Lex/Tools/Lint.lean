...
Initial check set (LX.5):
  ...
  2. Each `Lex/Inputs/<id>.json` parses successfully.
...
elaborator (`Lex.DSL.Law`) and `lex_codegen --check`.
...
def printHelp : IO UInt32 := do
  ...
  IO.println ""
  IO.println "Validates `Lex/IndexRegistry.txt` and the codegen-input JSON"
  IO.println "files under `Lex/Inputs/` for §13.1 well-"
