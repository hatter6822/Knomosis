...
| 1.30     | 2026-06-10 | Workstream GP.11.9 (gas-cost benchmarks for the v1.3 L1 operations) lands on the Solidity surface.  `solidity/test/BenchmarkGasV1_3.t.sol` adds 15 deterministic pure-call gas benchmarks across 7 scenario contracts (no fuzz; per-scenario state staged in `setUp`; companion `test_sanity_*` tests pin every scenario assumption — first-time vs repeat depositor nonces, the 15 ETH : 45 000 BOLD seeded reserve depth, staged exact approvals, Liquity branch shutdown states), covering `depositETHWithFee` / `depositBoldWithFee` (first + repeat shapes), `ammSwap` ETH→BOLD (first-BOLD-recipient + repeat) and BOLD→ETH, `closeBoldCircuit` / `openBoldCircuit` / `setBoldTvlCap` / `emergencyDisableAmm`, the Liquity auto-trigger's fast (first-branch) / worst (last-branch) / no-shutdown probe paths, and a plain-`depositETH` v1.0 reference row.  The committed baseline `solidity/test/BenchmarkGasV1_3.gas-snapshot` is regenerated via `make snapshot-gas` and enforced in CI by `make snapshot-gas-check` (`forge snapshot --check --tolerance 5` scoped by a shared `--match-contract BenchmarkGasV1_3 --match-test test_gas_` filter; any per-benchmark deviation beyond 5% in either direction fails, so regressions block and improvements are ratcheted into the baseline; the gate step runs in `.github/workflows/ci-solidity.yml` after `forge test` on every `solidity/**` PR).  Headline baselines (execution gas; end-user estimate ≈ +21k intrinsic): `depositETHWithFee` 62 401 first / 45 015 repeat; `depositBoldWithFee` 83 261 / 66 051; `ammSwap` ETH→BOLD 71 491 first-recipient / 54 303 repeat; BOLD→ETH 56 876; `closeBoldCircuit` 31 904; auto-trigger close 37 999 (first branch) / 53 400 (last branch); no-shutdown probe 34 444.  Cost-structure deltas match EVM storage economics exactly (first-interaction premium ≈ 17.1k = the zero→non-zero SSTORE surcharge; fee-split machinery overhead ≈ 8.5k over plain `depositETH`; BOLD-leg premium ≈ 20.9k for the `transferFrom` pull + delta reads + per-BOLD TVL accounting).  Operator-facing table, $-cost methodology ("at 30 gwei and $3 000/ETH a typical first fee-split deposit ≈ $7.5"), regeneration discipline, and mock-fidelity caveats land in `docs/gas_pool_runbook.md` §9.  `forge test` 815 passed / 0 failed / 12 skipped (24 new tests); no Lean / Rust source delta; no kernel TCB delta; no new axioms.  Version v0.5.8. |
