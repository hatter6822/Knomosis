...
| 1.29     | 2026-06-09 | Workstream GP.11.8 (AMM state-root commitment integration).  The `BridgeState` encoder (`Encoding/State.lean`) is extended to serialise the five AMM/BOLD state fields (`ammReserveEth`, `ammReserveBold`, `boldCircuitClosed`, `boldTvlCap`, `boldTotalLockedValue`) as trailing self-delimiting Nat segments (Bool encoded as `if b then 1 else 0`), and the decoder reconstructs them symmetrically.  The EI.7.e injectivity proof (`BridgeState.encode_injective`, `Encoding/BridgeInjective.lean`) is extended from a 3-way to an 8-way conjunction via two new helpers: `nat_encode_suffix_split` (chain-splits two `enc(n) ++ rest` equalities using the self-delimiting property of CBE Nat) and `bool_as_nat_injective` (injectivity of the `if b then 1 else 0` mapping).  The commitment scheme (`FaultProof/Commit.lean`) integrates the extended encoding: `ExtendedState.extEq` grows from 7 to 12 conjuncts (5 new AMM field equalities), `CanonicalBounds` adds 4 new `< 2^64` bound fields, and the composition theorem threads the new bounds to `encode_injective`.  Two GP.11.8 headline theorems: `bridgeState_commit_includes_ammState` (the encoding formula literally includes all 5 AMM fields — proved by `rfl`) and `bridgeState_commit_extends_v1_2` (v1.2 backward compatibility: identical core fields + genesis AMM defaults ⇒ identical commitment).  Both depend only on `{propext, Classical.choice, Quot.sound}` (no custom axioms; zero sorries).  15 acceptance tests (`faultproof-amm-commit` suite): genesis 32-byte commitment, each AMM field independently alters commitment (5), post-deposit / post-swap / post-circuit-close state-root changes (3), encoding distinguishes non-zero fields, term-level API stability for both theorems, determinism, v1.2 migration, and encoding round-trip.  `lake test` 2983 (149 suites); all audit gates green.  No kernel TCB delta, no new axioms.  Version v0.5.7. |
