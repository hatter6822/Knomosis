...
| 1.28     | 2026-06-08 | Workstream GP.11.7 (cross-stack AMM fixture corpus) lands tri-stack (Lean + Rust + Solidity).  The Lean generator `LegalKernel/Test/Bridge/CrossCheck/AmmSwap.lean` (`crosscheck-amm-swap`, 17 cases) emits a 66-entry corpus (54 grid + 12 corner) into both `solidity/test/CrossCheck/fixtures/amm_swap.json` (JSON) and `runtime/tests/cross-stack/amm_swap.cxsf` (binary CXSF tag 8).  The grid spans 3 reserve sizes (Small 10^12, Medium 10^15, Large 10^16) × 2 directions (ETH→BOLD, BOLD→ETH) × 3 swap sizes (1%, 10%, 50%) × 3 slippage thresholds (exact, 1% slack, 50% slack) = 54; the 12 corners cover zero reserves, zero amount, max-fee, both-zero, single wei, same-resource, and equal reserves.  All amount-scale fields and k-products emitted as `0x`-prefixed 32-byte BE hex strings via `hexFromUint256BE`.  The corpus verifies: CBE byte-equivalence (`encode_action` Lean == Rust for all 66 entries), `getAmountOut` formula compliance (Lean == Solidity `AmmMath.getAmountOut` == Rust u128 recomputation), k-monotonicity (`kBefore ≤ kAfter`), no-drain (`expectedOut < reserveOut`), slippage flag consistency, and header-constant agreement across all three stacks.  Rust consumer: `runtime/knomosis-l1-ingest/tests/cross_stack_amm_swap.rs` (8 tests).  Solidity consumer: `solidity/test/CrossCheck/AmmSwapFixtures.t.sol` (7 tests incl. two hand-vector anchors).  Hash-independent: no keccak-binding gate needed.  `FixtureKind::AmmSwap` (tag 8) added to `knomosis-cross-stack`.  `lake test` 2965 (148 suites); `cargo test --workspace` 1943; `forge test` green.  No kernel TCB delta, no new axioms.  Version v0.5.3. |
