...
| (GENESIS_PLAN §15E.12 + amendment 1.33) | SB — batched state-root submission + user-facing L2 AMM | 13 phases (SB.0–SB.12) | ~4 wks | **Complete** (v0.14.0).  One L1 record per batch + per-batch actions-root SMT + inclusion-authenticated terminate (~239 gas/action amortised at B=1000); `Laws.reserveSwap` (Action 25, Events 23/24) + the L2-primary seed-leg funding loop + opt-in deposit materialisation; fixed the revert dead end (R1/R3/R4) and the game→bridge revert gap (R6).  The Lean game-model actions-root anchor follow-up is CLOSED (`GameState.actionsRoot` + `anchored_challenger_wins`; audit-22 MAJOR closed at the model level); the remaining recorded follow-up (`docs/audits/19-findings-and-followups.md` SB close-out) is on-chain terminate signature verification. | builds on H + SVC + GP.11 (all Complete) |
