...
| M9 | **PARTIAL** — the path-order foundation | The first layer of M8's residue, and a debt of its own: `pathSorted (frontierOf …)` was a VALUE-level fact — checked on two example write sets — while `frontierShapeOk`'s whole argument rests on it ("strict ascent gives distinctness for free, which is why the shape check is one comparison rather than two").  Now proved.  `divBelow_agree_above` and `divBelow_eq_of` characterise the divergence; `pathLess_total` and `pathLess_trans` make path order a strict total order on keys the walk can tell apart (the latter is where `adjacent_div_ne` earns its keep — equal divergence levels are impossible for a sorted triple, so only the two strict orderings remain and each determines `div a c` outright); `pathSorted_frontierInsert` / `pathSorted_frontierOf` lift it to the frontier; and `pathSorted_keys_nodup` / `frontierOf_keys_nodup` turn sortedness into distinctness.  The side condition is `KeysSeparated` — the cells' keys differ at some bit the walk READS — phrased on `==` rather than `≠` so it is the same relation `frontierInsert` decides on, and discharged by computation on real cell keys in `faultproof-frontier`.  **One design correction fell out.**  `bundleValueAt` looked cells up by HASHED KEY while `KnomosisStepVMRoot._findOpened` looks them up by `(cellKind, keyA, keyB)` — the TAG.  The two agree on anything past the shape check, but they made the stacks decide one question two ways, and the by-key form made "the entry whose key matches IS the entry" a fact about `smtCellKey`'s injectivity rather than about a decidable equality on `CellTag`.  Lean now matches the contract, and `bundleValueAt_stepMultiBundle` — the bridge from the submitted bundle to `getCellValue`, which every `VerifierWrites` correctness theorem is stated against — follows.  **What remains** for `stepMultiPostRoot_eq_commit_productionApplyBudget`: a reader congruence for `plannedBalances` (the partial bundle reader agrees with `stateBalanceReader` wherever the derivations look), a `derivedCellValue_correct` composing the `*_correct` family over the twenty-five variants, and the discharge of `multiFold_eq_commit_post`'s well-formedness side conditions. |
