...
| M3 | The pre/post pair | `multiFoldPostRoot` and `multiFold_eq_commit_post`.  This SUBSUMES `foldStateCellWrites_eq_commit_of_coherent` — m keys at once, order-independent — and replaces it in CLAUDE.md's headline table rather than deleting it.  **The substantive piece is the m-key off-cell lemma**: the sibling list is built against the PRE-state's entries and must serve the POST-state's too, which holds because a gap sub-tree contains no opened cell.  That is S1's `dropKey_stateCellEntries_perm_of_agree_off` generalised from one key to a set (`dropKeys`), composed with `smtRootListAux_perm`.  Sizing it honestly before starting: this is the largest single proof in §6, and the pre/post pairing itself (`multiWalkPair` plus two projection theorems) is the easy half. |
