...
| M9e | **NOT DONE — deliberately, and the reason has changed** | M8 kept the chained write algebra (`stepWriteBundle` / `stepPostRoot`, `chainWrites`, `canonicalCellChain`, `foldStateCellWrites`, `CellWriteChain`, `ChainCoherent`, `dropKey_stateCellEntries_perm_of_agree_off`) on ONE stated ground: the multiproof's guarantee was only value-level, and retiring a headline theorem before its replacement is proved is the wrong order.  M9 removed that ground.  It did not establish the other one. The retirement case was "a second consensus surface with its own corpus columns is a drift risk" — and that risk is already gone: M8 deleted `writeBundleGoldens` / `stepPostRootGoldens`, and nothing on L1, in the Rust conduit or in the corpus consumes any of it today.  What is left is proved Lean that documents the write algebra and costs nothing.  Against deleting it: `StepWriteSets.lean` also hosts the per-variant `WriteSetComplete` proofs, and `stepMultiFold_eq_commit_post` CONSUMES `WriteSetComplete` (through `agreeOffOpened_openedOf`) — so the module stays either way and the cut runs through a live dependency rather than around a dead one.  **What DID change in the tests, because it is an improvement regardless:** `faultproof-terminate`'s headline case and `faultproof-terminate-bundle`'s three `expectedPostCommit` cases compared the multiproof against `stepPostRoot` — one verifier against another.  They now compare against `commitExtendedState (productionApplyBudget …)`, the published root of the state the step produces, computed with no reference to any fold.  That is the same target `stepMultiFold_eq_commit_post` proves, so the value-level and theorem-level checks now assert the same thing. |
