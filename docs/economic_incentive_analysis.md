...
|---|------|--------------------------|
| E-1 | **Parameter sizing is a deployment responsibility, not a proof.** Bonds, stakes, caps, fees are immutable constructor values; incentive-compatibility (IC-1…IC-6) depends entirely on choosing them correctly. | **`docs/deployment_parameters.md`** is the per-parameter sizing guide (every immutable constructor value → its IC condition + guard + guidance); the **calibration harness is shipped** — `scripts/economic_simulation.py` turns IC-1…IC-6 into a swept numeric envelope (min incentive-compatible bond per depth × gas price, anti-spam stake floor, v1-vs-v2 pool over-payment). |
