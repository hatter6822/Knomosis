...
The `runtime/canon-faultproof-observer` Rust crate (deferred;
see §H.10.5 of the workstream plan) is the recommended off-chain
observer.  Until it ships, operators should run the Lean-side
`LegalKernel.FaultProof.Observer` reference at audit cadence
(weekly minimum) to detect:
...
## 7. Rust observer crate specification (deferred to follow-up)
...
`LegalKernel.FaultProof.Observer`.
