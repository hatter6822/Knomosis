...
**Rust-side test count.**  ~1 495 tests across the 11 workspace
...
| `knomosis-l1-ingest`                | ~290  | RH-B L1 event watcher daemon + GP.6.1 fee-split mirror     |
...
Headline contributions surviving in current code:
  ...
  * **GP.6.1** Rust-side `knomosis-l1-ingest` GP-family encoder +
    ...
    workspace `cargo test --workspace --locked` reports ~1495 tests
    passing.
