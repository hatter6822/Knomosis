...
| GW | Gateway (HTTP/JSON + SSE) | In progress (G0.1–G0.3/G1.0/G1.1/G1.2/G1.4/G1.6a/G1.6b/G1.7/G1.8 complete; G1.9 next — `gateway_integration_plan.md`) |
...
indexer + upstream TCP probes), and G1.4 (the fail-closed
`subtle::ConstantTimeEq` bearer-token gate, applied before routing;
`/healthz` + `/readyz` exempt) are complete.  Next: the first shippable
read-only slice (G1.9 — read-path integration tests).
