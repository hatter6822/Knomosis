...
| GW | Gateway (HTTP/JSON + SSE) | In progress (read-only slice shipped + hardened; submit track complete; events track underway: G0.1–G0.3/G1.0–G1.4/G1.6a/G1.6b/G1.7/G1.8/G1.9/G2.1a/G2.1b/G2.2/G2.3/G2.4/G2.5/events track complete (G3.1/G3.2/G3.3 + the full G3.4 SSE fan-out (ring/mux/dispatch/resume) + G3.5 `/v1/events/stream` wiring); G4 hardening underway (G4.1 rate-limit (early via G1.3) + G4.3 observability + G4.4 graceful shutdown + G4.5 dep-audit complete; G4.2 TLS / G4.6 load / G4.7 runbook next); G2.1c pipelining + G3.2c cross-stack pin deferred — `gateway_integration_plan.md`) |
