...
**Test count.**  ~2 965 tests across 148 suites (the GP.11.7
cross-stack AMM fixture corpus adds the `crosscheck-amm-swap` suite,
17 cases — the 66-entry fixture generator (54 grid + 12 corner),
header-shape validation, per-entry u64 guard (`amountIn` / `expectedOut`
≤ u64::MAX), k-monotonicity (`kBefore ≤ kAfter`), no-drain
(`expectedOut < reserveOut`), CXSF binary corpus generation (tag 8),
grid/corner count agreement, and term-level API stability for the
`getAmountOut` / `getAmountOut_lt_reserveOut` / `k_nondecreasing`
theorems; the GP.11.6
...
further 13 cases (71 total then; 76 after GP.11.5) — the `gasPoolActor` = 1 /
...
(31) / `bridge-ingest` (28) value fixtures onto the GP.7.1 genesis id 3
(later advanced to 4 by GP.11.5, which reserves `ammReserveActor = 3`).
Earlier, the GP.7.0
...
**Rust-side test count.**  ~1 943 tests across the 11 workspace
crates (the GP.11.7 cross-stack AMM fixture corpus adds 8
`knomosis-l1-ingest` tests in `cross_stack_amm_swap.rs` —
`amm_swap_corpus_byte_equivalence` (Rust `encode_action` bytes ==
Lean `expectedCbe` for all 66 entries), `amm_swap_corpus_coverage`
...
further advanced to 4 by GP.11.5, below)
— is complete end-to-end (Lean + the Rust `knomosis-l1-ingest`
runtime adaptor, which mirrors the same genesis allocation so the
...
API stability for every headline theorem) — is complete on the Lean side, and
GP.11.5 — the `ammReserveActor` reservation (`ActorId 3`, the L2 reflection of
the L1 AMM reserves; the genesis `AddressBook.empty.nextActorId` advances 3 → 4
so a fresh deployment's first user is `ActorId 4`; three axiom-free
disjointness theorems `ammReserveActor_ne_bridgeActor` / `_ne_gasPoolActor` /
`_ne_sequencerActor` + the four-conjunct `empty_assign_id_avoids_reserved`
guarantee) — is complete end-to-end (Lean + the Rust `knomosis-l1-ingest`
runtime adaptor, which mirrors the genesis-4 allocation via
`INITIAL_NEXT_ACTOR_ID = 4` / `AMM_RESERVE_ACTOR_ID = 3` so the reservation is
honoured in production), and GP.11.6 — the `ammReserveActor` local policy
...
contributions surviving in current code:
  ...
  * **GP.7.1** Reserved gas-pool actors
    ...
    `SEQUENCER_ACTOR_ID` (2) constants — both Lean and Rust genesis were
    later advanced a further step to `4` by GP.11.5, reserving
    `ammReserveActor = 3`; see the GP.11.5 snapshot below), so the adaptor
    that performs the actual `assign` honours the reservation — a fresh L1
    identity registration is issued the first non-reserved id, never a
    reserved slot.  The
  ...
  * **GP.11.5** `ammReserveActor` reservation
    (`LegalKernel/Bridge/BridgeActor.lean` +
    `LegalKernel/Bridge/AddressBook.lean`).  Reserves `ActorId 3` for
    `ammReserveActor` — the L2-side counterpart of the L1
    `KnomosisBridge`'s `ammReserveEth` / `ammReserveBold` storage slots
    (GP.11.1 / GP.11.2 / GP.11.3) — and advances the genesis
    `AddressBook.empty.nextActorId` from `3` (GP.7.1) to `4`, so the
    first user a fresh deployment registers is `ActorId 4`.  Shipped:
    - The `ammReserveActor : ActorId := 3` constant; its balances are
      mutated only by bridge-attested `ammSwap` actions (frozen `Action`
      index 23): an `ammSwap` credits this actor at the swap's
      `fromResource` and debits it at the `toResource`, mirroring the
      on-chain reserve update (the `Action.ammSwap`'s reserve-actor field
      is this constant in a production deployment).
    - Three axiom-free `decide` disjointness theorems
      (`ammReserveActor_ne_bridgeActor` / `_ne_gasPoolActor` /
      `_ne_sequencerActor`), guaranteeing the AMM-reserve ledger is a
      distinct accounting domain (an `ammSwap` never touches the
      gas-pool / sequencer balances).
    - `AddressBook.empty.nextActorId = 4`, with the existing
      `addressBook_empty_nextActorId` theorem updated in place to `= 4`
      (the single source of truth — NOT a `_v1_3`-suffixed new theorem;
      version-marker identifiers are forbidden by `naming_audit`), and
      the `empty_assign_id_avoids_reserved` reservation guarantee
    ...
    - **Rust lockstep** (`runtime/knomosis-l1-ingest`, pulled forward
      ...
      identity registration is issued `ActorId 4`, never the reserved AMM
      slot.  This is load-bearing for correctness: leaving the adaptor at
      `3` while Lean reserves `3` would assign the first user `ActorId 3`,
      colliding with `ammReserveActor` in production.  The state-file
      replay's reserved-range rejection now covers id `3` (a state file
      from a GP.7.1-era deployment that legitimately allocated id 3 to a
      user is rejected on upgrade with the actionable GP.10.4-migration
      diagnostic — `replay_rejects_newly_reserved_amm_actor_id`); the
      `l1_ingest.cxsf` cross-stack corpus and all `address_book` /
      `state` / `translation` / `watcher` / integration tests are rebased
      onto the genesis-4 allocation.  This is the *fresh-genesis* half;
      the migration of *existing* deployments that already allocated a
      user in slot 3 remains Phase GP.10.4.  No kernel TCB delta, no new
      axioms (the disjointness theorems are axiom-free; the genesis +
      reservation theorems depend only on `{propext, Classical.choice,
      Quot.sound}` via `Std.TreeMap`).
  * **GP.11.6** `ammReserveActor` local policy
    (`LegalKernel/Bridge/AmmReservePolicy.lean`).  The per-actor
    `LocalPolicy` (Workstream LP) the admission layer consults whenever
    `ammReserveActor` (`ActorId 3`) signs an action.  Restricts the
    reserve's outflow to a single capability: `ammSwap` (frozen Action
    index 23).  Single-clause `denyTags ammReserveDeniedTags` where
    `ammReserveDeniedTags = List.range 24 |>.filter (· ≠ 23)` = `[0..22]`;
    the deny-list's coverage is mechanically enforced by the existing
    `Action.tag_lt_denyListBound` forcing function (exhaustive `cases` —
    a 25th constructor without bumping the range breaks the proof).
    Headline theorems: `ammReservePolicy_denies_all_non_ammSwap` (the
    reserve can never transfer / mint / burn / deposit / top up budgets /
    sign any non-ammSwap action), `ammReservePolicy_permits_ammSwap`
    (the reserve CAN sign an `ammSwap`),
    `ammReservePolicy_permits_iff` (single source-of-truth
    characterisation: permitted iff `Action.tag = 23`).
    **Admission-layer reach + LP.7 meta-action escape hatch:**
    `ammReservePolicy_admission_permits_meta_actions` PROVES the
    structural limitation, motivating the complementary
    `ammReserveAuthorityPolicy` (an `AuthorityPolicy`, intersected into
    the deployment policy at genesis).  The `AuthorityPolicy` conjunct
    has NO meta-action exemption, so
    `ammReserveAuthorityPolicy_rejects_meta` bars the escape hatch,
    while `ammReserveAuthorityPolicy_rejects_non_ammSwap` /
    `ammReserveAuthorityPolicy_authorizes_ammSwap` additionally
    enforce the action-restriction at the authority layer (the reserve
    can ONLY sign `ammSwap` — its sole legitimate mutation path).
    `ammReserveAuthorityPolicy_other_actors_unrestricted` /
    `ammReserveAuthorityPolicy_intersect_rejects_meta` prove the genesis
    intersection narrows ONLY `ammReserveActor` and bars its
    meta-actions under ANY base policy.
    **Genesis wiring:** `AmmReserveGenesis` structure +
    `ammReserveGenesis` constructor bundle BOTH halves
    (`ammReserveGenesisState` declares the policy,
    `ammReserveGenesisPolicy` intersects the authority policy) so
    neither can be wired without the other
    (`ammReserveGenesis_wires_both_halves`).  Contract theorems:
  ...
  * **GP.11.7** Cross-stack AMM fixture corpus — the tri-stack
    ...
    `crosscheck-amm-swap`, 17 cases) authors BOTH a rich JSON fixture
    ...
    `FixtureKind::AmmSwap` / on-disk tag 8) from ONE 66-entry list:
    ...
    `slippage ∈ {exact, 1% slack, 50% slack}`, plus 12 corner-case
    entries (zero reserves, zero amount, max-fee, both-zero, single
    wei, same-resource, equal reserves).  Reserve sizes are scaled
    to u64-safe ranges satisfying the CBE encoding constraint
    ...
    − expectedOut)), `ammReserveActor`, and `expectedCbe` (Lean's
    `Encoding.Action.encode` of the corresponding `Action.AmmSwap`).
    Header: `workstream "GP.11.7"`, `count 66`, `gridCount 54`,
    `cornerCount 12`, `ammSwapFeeBps 30`, `bpsDenominator 10000`,
    ...
    assertion (`kBefore ≤ kAfter` for every entry), and a no-drain
    assertion (`expectedOut < reserveOut` for every valid entry).
    The Rust consumer
    (`runtime/knomosis-l1-ingest/tests/cross_stack_amm_swap.rs`, 8
    tests) byte-matches `encode_action` against Lean's `expectedCbe`
    for all 66 entries, recomputes `getAmountOut` in u128 checked
    arithmetic (skipping entries whose intermediates overflow u128,
    asserting >50% checked), and independently verifies
    k-monotonicity (via u256 pair comparison), no-drain, slippage
    consistency, header constants, tag pin (tag 23), and corpus
    coverage (grid + corner).  The Solidity consumer
    (`solidity/test/CrossCheck/AmmSwapFixtures.t.sol`, 7 tests)
    ...
    entry), and two hand-vector anchors (`no-fee half-pool` /
...
strong-conservation extension (the `ammReserveActor` reservation it
needs has now landed under GP.11.5, below; the conservation extension
itself remains follow-up work); the materialised
