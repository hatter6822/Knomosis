...
//! offset  size  field
//! ------  ----  --------------------------------------------
//!     0    4    MAGIC ASCII "KNOM" (0x4B 0x4E 0x4F 0x4D)
...
pub const FRAME_MAGIC_0: u8 = 0x4B;

/// Frame magic byte 2: 'N' (0x4E).
pub const FRAME_MAGIC_1: u8 = 0x4E;

/// Frame magic byte 3: 'O' (0x4F).
pub const FRAME_MAGIC_2: u8 = 0x4F;

/// Frame magic byte 4: 'M' (0x4D).
pub const FRAME_MAGIC_3: u8 = 0x4D;
...
pub enum TailError {
    ...
    /// `"KNOM"`.  Indicates either a corrupt log file or an
    /// attempt to tail a file that isn't a Knomosis log.
    #[error("bad magic at offset {offset}: got {got:02x?}, expected [4b 4e 4f 4d]")]
    BadMagic {
