...
//! parallel).  The request-body size limit lands with G2.2 (the first
//! body-consuming endpoint); per-connection read/idle timeouts are
...
///
/// Binds the listener, then spawns a bounded pool of
/// `config.handler_threads` worker threads (the concurrency governor —
/// [`spawn_handler_pool`]) and blocks until a worker exits.  Under
/// normal operation the workers loop forever; this only *returns* on a
/// bind/spawn failure.  Graceful shutdown is G4.4.
///
/// # Errors
///
...
pub fn serve(config: &Config) -> Result<(), ServeError> {
    ...
    let handles = spawn_handler_pool(&server, config.handler_threads)?;
...
/// [`handle_request`].  This is the gateway's concurrency governor: it
/// caps the number of requests processed in parallel.  Returns the
/// workers' join handles.
///
...
fn worker_loop(server: &tiny_http::Server) {
    loop {
        match server.recv() {
...
/// `"OTHER"`, which the router answers as 405 + `Allow` on a known
/// path (or 404 on an unknown one).
