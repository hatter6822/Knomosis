 ...
 * This file ships small non-inline wrappers (`knomosis_lean_*`)
 * whose ONLY job is to forward to the underlying inlines so the
 * Rust side has a stable symbol surface to bind to.
 *
 * The actual Lean ABI entry point `knomosis_verify_ecdsa` is
 ...
 * owner).  The Rust-side `knomosis_verify_ecdsa` decrements the
