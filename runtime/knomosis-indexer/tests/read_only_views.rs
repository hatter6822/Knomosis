...
fn balance_view_reads_over_read_only_handle() {
    ...
    assert_eq!(view.get(7, 0).unwrap(), Amount::from_u64(1_000));
    assert_eq!(
        view.get(7, 1).unwrap(),
        Amount::from_u64(0),
    ...
    assert_eq!(
        view.scan_all().unwrap(),
        vec![(7, 0, Amount::from_u64(1_000))]
    );
    drop(writer);
}
...
fn budget_view_reads_over_read_only_handle() {
    ...
    assert_eq!(
        view.get_actor_budget_current_epoch_grants(7).unwrap(),
        Amount::from_u64(100)
    );
    assert_eq!(
        view.get_actor_budget_current_epoch_consumed(7).unwrap(),
        Amount::from_u64(30)
    );
    // free_tier 50: remaining = 50 + 100 − 30 = 120.
    assert_eq!(
        view.remaining_this_epoch(7, 50).unwrap(),
        Amount::from_u64(120)
    );
    // An actor with no budget cells: remaining = free_tier only.
    assert_eq!(
        view.remaining_this_epoch(999, 50).unwrap(),
        Amount::from_u64(50)
    );
    drop(writer);
}
