...
//! chaos_with_seed -- --test-threads=1` with `KNOMOSIS_CHAOS_SEED=N`
//! for each N).
//!
//! ## Test discipline
//!
...
/// Deterministic seed helper.  Reads `KNOMOSIS_CHAOS_SEED` env var
/// for reproducibility; defaults to 0.
fn chaos_seed_for_test() -> u64 {
    std::env::var("KNOMOSIS_CHAOS_SEED")
        .ok()
...
/// `KNOMOSIS_CHAOS_SEED=0..=9` to satisfy the workstream plan's
