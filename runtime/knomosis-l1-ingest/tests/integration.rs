...
fn state_persists_across_restarts() {
    ...
    // last_confirmed_block = Some(0).
    {
        ...
        // (fresh id 4, post-GP.11.5).
        let addr = EthAddress::from_bytes(&[0x33; 20]).unwrap();
        assert_eq!(watcher2.address_book().lookup(&addr), Some(4));
    }
}
...
fn end_to_end_eip1271_registration() {
    ...
    match &recorded[0].unsigned.action {
        knomosis_l1_ingest::action::Action::RegisterIdentity { actor: id, pk } => {
            // Fresh registration is issued id 4 post-GP.11.5.
            assert_eq!(*id, 4);
    ...
    // Address book has the (actor, 4) mapping.
    let addr = knomosis_l1_ingest::action::EthAddress::from_bytes(&actor).unwrap();
    assert_eq!(watcher.address_book().lookup(&addr), Some(4));
}
