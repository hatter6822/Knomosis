...
mod tests {
    ...
    fn encode_deposit_with_fee_layout() {
        ...
        assert_eq!(encoded[27], CBE_TAG_UINT);
        assert_eq!(&encoded[28..36], &2u64.to_le_bytes());
        // user_amount (1000) at offset 36, on the amount head.
        assert_eq!(encoded[36], CBE_TAG_AMOUNT);
        assert_eq!(&encoded[37..53], &1000u128.to_le_bytes());
        // pool_amount (500) at offset 53, on the amount head.
        assert_eq!(encoded[53], CBE_TAG_AMOUNT);
        assert_eq!(&encoded[54..70], &500u128.to_le_bytes());
        // budget_grant (10) at offset 70 — a UNIT count, narrow head.
        assert_eq!(encoded[70], CBE_TAG_UINT);
        assert_eq!(&encoded[71..79], &10u64.to_le_bytes());
        // deposit_id (42) at offset 79.
        assert_eq!(encoded[79], CBE_TAG_UINT);
        assert_eq!(&encoded[80..88], &42u64.to_le_bytes());
    }
    ...
    fn encode_top_up_action_budget_layout() {
        ...
        assert_eq!(encoded[9], CBE_TAG_UINT);
        assert_eq!(&encoded[10..18], &0u64.to_le_bytes());
        // gas_amount (100) on the amount head at offset 18.
        assert_eq!(encoded[18], CBE_TAG_AMOUNT);
        assert_eq!(&encoded[19..35], &100u128.to_le_bytes());
        // budget_increment (5) — a UNIT count, narrow head at 35.
        assert_eq!(encoded[35], CBE_TAG_UINT);
        assert_eq!(&encoded[36..44], &5u64.to_le_bytes());
        // pool_actor (2) at offset 44.
        assert_eq!(encoded[44], CBE_TAG_UINT);
        assert_eq!(&encoded[45..53], &2u64.to_le_bytes());
    }
    ...
    fn encode_top_up_action_budget_for_layout() {
        ...
        // gas_amount (100) on the amount head at offset 27.
        assert_eq!(encoded[27], CBE_TAG_AMOUNT);
        assert_eq!(&encoded[28..44], &100u128.to_le_bytes());
        // budget_increment (5) — a UNIT count, narrow head at 44.
        assert_eq!(encoded[44], CBE_TAG_UINT);
        assert_eq!(&encoded[45..53], &5u64.to_le_bytes());
        // pool_actor (2) at offset 53.
        assert_eq!(encoded[53], CBE_TAG_UINT);
        assert_eq!(&encoded[54..62], &2u64.to_le_bytes());
    }
    ...
    fn encode_deposit_with_fee_known_vector() {
        ...
        let expected: Vec<u8> = vec![
            ...
            // user_amount 1000 = 0x03e8 on the 17-byte amount head.
            0x01, 0xe8, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, // pool_amount 500 = 0x01f4 on the 17-byte amount head.
            0x01, 0xf4, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, // budget_grant 10 = 0x0a — a UNIT count, 9-byte head.
    ...
    fn encode_top_up_action_budget_known_vector() {
        ...
        let expected: Vec<u8> = vec![
            ...
            // gas_amount 100 = 0x64 — wei-denominated, so the 17-byte
            // amount head (tag 0x01 + 16 LE bytes).
            0x01, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, //
            // budget_increment 5 — a UNIT count, so the 9-byte head.
    ...
    fn encode_top_up_action_budget_for_known_vector() {
        ...
        let expected: Vec<u8> = vec![
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            // gas_amount 100 = 0x64 on the 17-byte amount head.
            0x01, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, //
            // budget_increment 5 — a UNIT count, so the 9-byte head.
            0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // pool_actor 2.
    ...
    fn encode_claim_budget_refund_known_vector() {
        ...
        let expected: Vec<u8> = vec![
            ...
            // wei_per_budget_unit 5 — a wei RATE, so the amount head.
            0x01, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, // pool_actor 2.
            0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];
        assert_eq!(actual, expected);
    }
    ...
    fn encode_amm_swap_known_vector() {
        ...
        let expected: Vec<u8> = vec![
            ...
            0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            // amountIn 1000 = 0x03e8 on the amount head
            0x01, 0xe8, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, // amountOut 500 = 0x01f4 on the amount head
            0x01, 0xf4, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, // ammReserveActor 3
    ...
    fn encode_reclaim_amm_reserves_known_vector() {
        ...
        let expected: Vec<u8> = vec![
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            // amount 5000 = 0x1388 on the amount head
            0x01, 0x88, 0x13, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, // reserveActor 3
    ...
    /// kernel can hold.  On the 17-byte amount head they encode
    ...
    fn value_fields_encode_at_full_width() {
        /// Assert `bytes` contains `value` as a 17-byte amount head at
        /// `offset`, i.e. the tag then the full 16-byte LE payload.
        fn assert_amount_at(bytes: &[u8], offset: usize, value: u128, what: &str) {
            assert_eq!(bytes[offset], CBE_TAG_AMOUNT, "{what}: amount tag");
            assert_eq!(
                &bytes[offset + 1..offset + AMOUNT_HEAD_LEN],
                &value.to_le_bytes(),
