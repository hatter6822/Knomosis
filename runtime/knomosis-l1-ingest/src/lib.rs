...
//! relevant `KnomosisBridge.sol` / `KnomosisIdentityRegistry.sol` events
...
//! ## Cross-stack corpus
//!
//! Three kinds of fixtures bind this crate's encoders to the Lean
//! reference:
//!
//!   * `runtime/tests/cross-stack/l1_ingest.cxsf` —
//!     `FixtureKind::L1Ingest`.  Each record's `input` is a
//!     serialised `(IngestedEvent, AddressBook snapshot,
//!     current_nonce)` triple; the `expected` field is the
//!     CBE-encoded Action that the Rust translator must produce.
//!     Regenerated by `examples/gen_ingest_fixtures.rs`.
//!   * `runtime/tests/cross-stack/l1_ingest_fee_split.cxsf` —
//!     `FixtureKind::L1IngestFeeSplit` (Workstream GP.6.1).  249
//!     records sweeping the fee-split parameter space; each
//!     `input` is a 58-byte `FeeSplitInput` and each `expected` is
//!     the CBE-encoded `Action::DepositWithFee` for the derived
//!     split.  Regenerated by `examples/gen_fee_split_fixtures.rs`
//!     (which also offers a `--check` drift gate run in CI).
//!   * `solidity/test/CrossCheck/fixtures/deposit_with_fee_action.json`
//!     (Workstream GP.6.1).  The genuine Lean → Rust differential:
//!     its `expectedCbe` field is computed by Lean's
//!     `Encoding.Action.encode` for the three GP-family
//!     constructors (`depositWithFee` / `topUpActionBudget` /
//!     `topUpActionBudgetFor`); the Rust consumer
//!     (`tests/cross_stack_lean_action.rs`) byte-matches its own
//!     `encode_action` against it.  Generated Lean-side by
//!     `LegalKernel/Test/Bridge/CrossCheck/DepositWithFeeAction.lean`.
//!
//! CI verifies byte-equality against the committed corpora on
//! every PR.
