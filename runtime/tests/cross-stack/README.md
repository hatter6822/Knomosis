...
### Fixture-kind details
  ...
  * **`L1IngestBold`** (GP.6.5) — 163 records.  `(input bytes =
    ...
    four-rate `wei_per_budget_unit ∈ {1, 10⁹, 3·10¹⁵, 10¹⁸}`) plus 3
    single-leg boundary entries; the Rust consumer
    `tests/cross_stack_bold.rs` independently recomputes both halves
    and byte-matches.  The committed file's drift is gated by `lake
    test`'s verify-mode re-write (the Lean-side fixture authority),
    not a Rust `--check` example.
