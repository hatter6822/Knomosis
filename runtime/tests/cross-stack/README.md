...
### Fixture-kind details
  ...
  * **`L1IngestBold`** (GP.6.5) — 190 records.  `(input bytes =
    ...
    Lean's `Encoding.Action.encode` + `ActorBudget.encode`.  A
    160-entry ETH+BOLD grid (over `amount ∈ {1, 10⁹, 10¹⁵, 10¹⁸}`,
    `chosen_fee_bps ∈ {0, 100, 1000, 2500, 5000}`, and the full
    four-rate `wei_per_budget_unit ∈ {1, 10⁹, 3·10¹⁵, 10¹⁸}`) plus 6
    boundary entries (each `u64::MAX` / `10¹⁸` clamp case mirrored on
    both legs) plus 24 USD-calibrated cross-amount entries (12
    ETH/BOLD pairs that deposit different amounts at calibrated rates
    yet yield equal budget grants — the spec's calibration-parity
    deliverable); the Rust consumer `tests/cross_stack_bold.rs`
    independently recomputes both halves and byte-matches.  The
