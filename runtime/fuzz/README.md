...
| `observer_terminate_bundle_json` | `knomosis-faultproof-observer` terminate-bundle intake — the JSON a `knomosis export-terminate-bundle` subprocess writes, carrying a MULTIPROOF whose three unbounded fields (the frontier array, the gap mask, the packed siblings) vary independently and constrain each other, plus the all-or-nothing batch binding (Workstream SB: the 65-byte signature and the inclusion wire against the batch's actions root) | `knomosis_faultproof_observer::strategy::parse_terminate_bundle_json` |
