...
bytes (102 bytes: four 9-byte uint heads — tag, fromResource,
toResource, ammReserveActor — plus two 33-byte amount heads). -/
...
def tests : List TestCase :=
  ...
  , { name := "GP.11.7: every entry's expectedCbe is 102 bytes (206 hex chars with 0x prefix)"
    , body := do
        for e in entries do
          let a : Action :=
            .ammSwap (UInt64.ofNat e.fromResource) (UInt64.ofNat e.toResource)
              e.amountIn e.expectedOut (UInt64.ofNat ammReserveActorId)
          let hex := encodeActionHex a
          if hex.length ≠ 206 then
            throw <| IO.userError s!"CBE length mismatch for {e.category}: {hex.length} ≠ 206"
    }
