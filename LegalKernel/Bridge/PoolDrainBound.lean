...
def Action.doesNotDebitPoolAt (rLeg : ResourceId) (signer : ActorId) : Action → Prop
  ...
  -- `gasPoolActor`.  A pool-DEBITING refund (`gr = rLeg ∧ pa =
  -- gasPoolActor`) is therefore NOT `doesNotDebitPoolAt`, so it cannot
  -- be a step of a `PoolBoundedTrace`: the GP.7.3 per-trace cap
  -- (`n × maxDrainPerAction`) ranges over the SEQUENCER-claim outflow
  -- discipline only.  The refund outflow is bounded SEPARATELY by the
  -- GP.9.1 admission gate — per-action by pool solvency
  -- (`getBalance gasPoolActor ≥ budgetUnits × weiPerBudgetUnit`) and
